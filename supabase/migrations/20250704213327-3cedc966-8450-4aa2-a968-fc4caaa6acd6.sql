-- Create community role enum (if not exists)
DO $$ BEGIN
...
ALTER COLUMN role TYPE community_role USING role::community_role;

-- Set default value for role
ALTER TABLE public.community_members 
ALTER COLUMN role SET DEFAULT 'member'::community_role;
...
DROP POLICY IF EXISTS "Community moderators can manage posts" ON public.posts;
