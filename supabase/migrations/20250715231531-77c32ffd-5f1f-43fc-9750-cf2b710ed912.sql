...
-- Create separate triggers for INSERT and DELETE operations
CREATE TRIGGER trigger_update_community_post_count_insert
  AFTER INSERT ON public.posts
  FOR EACH ROW
  WHEN (NEW.community_id IS NOT NULL)
  EXECUTE FUNCTION public.update_community_post_count();

CREATE TRIGGER trigger_update_community_post_count_delete
  AFTER DELETE ON public.posts
  FOR EACH ROW
  WHEN (OLD.community_id IS NOT NULL)
  EXECUTE FUNCTION public.update_community_post_count();

CREATE TRIGGER trigger_update_community_event_count_insert
  AFTER INSERT ON public.events
  FOR EACH ROW
  WHEN (NEW.community_id IS NOT NULL)
  EXECUTE FUNCTION public.update_community_event_count();

CREATE TRIGGER trigger_update_community_event_count_delete
